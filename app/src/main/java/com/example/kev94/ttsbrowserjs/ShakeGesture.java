 ...
 */
public class ShakeGesture implements iGestures, SensorEventListener {

    //constants for the shake gesture
    public static final double SHAKE_THRESHOLD = 3;
        ...
        if(!mGestureCompleted) {
            ...
            if (Math.abs(event.values[0]) > SHAKE_THRESHOLD) {
                if ((event.timestamp - mCurrentTimestamp) > SHAKE_MAX_EVENT_TIME) {
                    Log.i("Gesture", "Value: " + Double.toString(event.values[0]) + " shakes: " + Integer.toString(mShakeCount) + " time: " + Long.toString(event.timestamp - mCurrentTimestamp));
